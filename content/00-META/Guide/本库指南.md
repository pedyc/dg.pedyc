...
7. **三层粒度**：领域（area，责任）→ 专题（MOC，知识地图）→ 原子（concept/atomic，事实），避免过宽与过碎两个极端
...
├── 30-RESOURCES/       # 外部资源（剪藏网页等）
├── 40-ARCHIVE/        # 归档（已完成/过时的内容、或者信噪比较低的笔记例如孤立节点）
├── 50-ZETTELCASTEN/   # 原创笔记（已内化的知识）
...
| 20-AREAS | 领域笔记 | `area` |
| 30-RESOURCES | 整合笔记 | `concept`, `moc`, `sop`, `question`, `term`, `comparison`, `record`, `roadmap`, `person` |
| 40-ARCHIVE | 归档内容 | - |
| 50-ZETTELCASTEN | 原子笔记 | `atomic` |
...
- **content-type 决定目录**：特定型（project/area/diary/article）由物理位置决定，通用型（atomic → 50-ZETTELCASTEN，其他 → 30-RESOURCES）
...
需要做事 → 创建项目(P-) → 关联领域(A-) → 项目/领域发展中发散观点 → 形成原创笔记
     ↓
项目结束 / 领域不活跃 → 归档(40-ARCHIVE)
```

- **触发**：有明确要做的事情
- **路径**：项目 → 领域 → 原创笔记
- **补充来源**：50-ZETTELCASTEN（原创）和 30-RESOURCES（外部）
...
flowchart TB
    ...
    subgraph 内容分层
        RAW[50-ZETTELCASTEN<br/>已内化的洞见]
        WIKI[30-RESOURCES<br/>Wiki 层整合]
        ARCHIVE[40-ARCHIVE<br/>归档]
    end
...
| **分层**  | 原子笔记 → Wiki → Archive   | 核心理念 + LLM Wiki 系统     | 知识存放位置和状态 |
...
| 层级 | 位置 | 说明 |
|------|------|------|
| 原子笔记层 | `50-ZETTELCASTEN/` | 已内化的洞见，source of truth |
| Wiki 层 | `30-RESOURCES/` | concept/moc/sop/term/comparison，LLM 维护 |
| Archive | `40-ARCHIVE/` | 过时知识 |
