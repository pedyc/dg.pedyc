...
- [2026-08-19] ingest | 新建 record「模拟面试02」
	- 位置：30-RESOURCES/模拟面试02.md
	- 父页面：[[求职]]
	- 覆盖：前端岗位模拟面试第 2 次，15 题（登录/JWT 存储、浏览器缓存、大文件上传、SSE、接口降级、性能指标、构建工具与 Vue3 原理）
	- 更新：wiki-index（Records 4→6，同步补入「模拟面试01」）、求职（FAQ 区）
- [2026-08-19] lint | 健康检查（full 运行，脚本 + 别名复核）
	- 孤儿页面 0；过期引用（归档被 wiki 层引用）0；矛盾 0（未发现）
	- 索引一致性：修复 wiki-index 断裂链接 1 处（[[简历素材]] → [[R-简历素材]]）
	- 概念缺口 4 个（atomic 提及≥2 次且无页面）：变量提升(3x)、localStorage(2x)、服务端渲染(2x)、Scope(2x)——待用户判断价值后创建/重定向
	- 误报澄清：MOC-Angular面试题 / SOP-实现网页斑驳光背景效果 / SOP-使用WebSocket实现实时双向通信 / SOP-Vue3-混合文件夹结构 / A-知识管理 / 数字花园概述 均经别名可解析
- [2026-08-19] ingest | 新建 question「登录页面怎么做」
	- 位置：30-RESOURCES/登录页面怎么做.md（纯标题，前缀 Q- 在 aliases）
	- 父页面：[[前端场景实践]]
