...
- [[浏览器通过事件循环机制处理异步任务]]
	- **原理**：事件循环持续监控执行栈和任务队列，当执行栈为空时从队列中取出任务执行，确保异步回调不会阻塞主线程
- [[await关键字是让出线程的标志]]
...
4. **GUI渲染**：浏览器会检测当前是否需要进行GUI渲染，如果不满足则跳过
...
> 在微任务执行过程中产生的微任务，会在当前Tick（同一轮微任务清空阶段）中连续执行完毕
...
// 执行输出：
script start // 同步代码从上往下执行
async1 start // await是让出线程的标志，await语句后的代码才是微任务
async2
promise1 
script end // 同步代码执行完毕，开始清空微任务队列
async1 end
promise2 // 微任务队列清空，开始执行宏任务队列
setTimeout
```
