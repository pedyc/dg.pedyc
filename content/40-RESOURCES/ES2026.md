...
graph TD
    ...
    E --> E2[Array.fromAsync<br/>数组构造]
    E --> E3[Iterator.concat<br/>迭代器串联]
    F --> F1[Uint8Array 编解码<br/>Base64/Hex]
    F --> F2[JSON.rawJSON<br/>源文本访问]
    G --> G1[#record/#tuple<br/>不可变数据]
    G --> G2[Math.sumPrecise<br/>精确求和]
    G --> G3[getOrInsert<br/>Upsert 模式]
```
...
t- **Map upsert**：`Map.getOrInsert()` 适用于缓存初始化、配置管理、计数统计等场景
- ⛔ **误用**
...
- **子级概念**：
	...
	- [[Math.sumPrecise]] — ES2026 精确求和
	- [[Map.getOrInsert]] — ES2026 Map upsert
