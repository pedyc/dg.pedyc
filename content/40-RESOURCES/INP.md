...
- [[INP测量完整交互周期|INP 测量的是交互的完整周期]]
	- **原理**：INP = 输入延迟 + 事件处理时间 + 绘制时间，而 FID 仅测量输入延迟
- [[长任务会阻塞主线程导致INP变差]]
	- **原理**：主线程被占用时，浏览器无法处理用户交互和绘制下一帧
...
[^1]: 即 100 份数据中采用第 98 份数据作为标准，例如页面上产生了一百次点击，INP 记录了每次延迟，将这 100 个值从小到大排序，第 98 百分位就是第 98 个值。假设排序后的数据如下：`[45,52,58, … 180,190,210,350,600]`，第 98 百分位就是 210。这个值意味着 `98%的交互响应时间<=220ms`
