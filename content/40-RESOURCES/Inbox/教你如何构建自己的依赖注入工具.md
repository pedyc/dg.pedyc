---
title: 教你如何构建自己的依赖注入工具
author: ["[[字节前端字节前端的技术实践分享 |微信公众号：字节前端ByteFE]]"]
description: 作者：华桥阅读前准备在阅读这篇文档之前，你可以先了解一下这些知识，方便跟上文档里面的内容： 概念知识：控制反转、依赖注入、依赖倒置；技术知识：装饰器 Decorator、反射 Reflect；TSyringe 的定义：Token、P…
tags: [clippings, "设计模式/依赖注入"]
date-created: 2026-05-26
date-modified: 2026-05-26
created: 2026-05-26
published:
source: https://zhuanlan.zhihu.com/p/667175246
---
...
- **TSyringe 的定义：Token、Provider** （ [github.com/microsoft/ts](https://link.zhihu.com/?target=https%3A//github.com/microsoft/tsyringe%23injection-token) ） **。**

本篇文章所有实现的实践都写到了 codesandbox 里面，感兴趣可以点进去看看源码： [codesandbox.io/s/di-pla](https://link.zhihu.com/?target=https%3A//codesandbox.io/s/di-playground-oz2j9) 。
...
这样的确满足了比较远的学生驾车去上学的需求，但是这里有一个问题，学生也有自己的具体选择和偏好，有的人喜欢宝马、有的人喜欢 [特斯拉](https://zhida.zhihu.com/search?content_id=236390917&content_type=Article&match_order=1&q=%E7%89%B9%E6%96%AF%E6%8B%89&zhida_source=entity) 。我们可以为了解决这样的问题继续使用继承的方式，继续抽象，得到喜欢宝马的有钱学生、喜欢特斯拉的有钱学生。

大家估计也会觉得这样写代码完全不可行， [耦合度](https://zhida.zhihu.com/search?content_id=236390917&content_type=Article&match_order=1&q=%E8%80%A6%E5%90%88%E5%BA%A6&zhida_source=entity) 太高，每个类型的学生在抽象的时候都直接和一个具体的交通工具绑定在一起。学生拥有的交通工具并不是这个学生创建出来决定的，而是根据他的家庭状况、喜好确定他使用什么样子的交通工具去上学；甚至家里可能有很多的车，每天看心情开车去上学。

那么为了降低 [耦合性](https://zhida.zhihu.com/search?content_id=236390917&content_type=Article&match_order=1&q=%E8%80%A6%E5%90%88%E6%80%A7&zhida_source=entity) ，根据具体的状态和条件进行依赖的创建，就要说到下面的模式了。

## 控制反转

**控制反转** （Inversion of Control，缩写为 **IoC** ）是一种设计原则，通过反转程序逻辑来降低代码之间的耦合性。

**控制反转容器** （IoC 容器）是某一种具体的工具或者框架，用来执行从内部程序反转出来的代码逻辑，从而提高代码的复用性和 [可读性](https://zhida.zhihu.com/search?content_id=236390917&content_type=Article&match_order=1&q=%E5%8F%AF%E8%AF%BB%E6%80%A7&zhida_source=entity) 。我们常常用到的 DI 工具，就扮演了 IoC 容器的角色，连接着所有的对象和其依赖。

![[../../_resources/教你如何构建自己的依赖注入工具/82c2227e233677b1225e6c6cfbc88d1c_MD5.webp]]

> 参考 Martin Fowler 关于控制反转和依赖注入的文章： [martinfowler.com/articl](https://link.zhihu.com/?target=https%3A//martinfowler.com/articles/injection.html)
...
- 基于 [构造函数](https://zhida.zhihu.com/search?content_id=236390917&content_type=Article&match_order=1&q=%E6%9E%84%E9%80%A0%E5%87%BD%E6%95%B0&zhida_source=entity) 。实现特定参数的构造函数，在新建对象时传入所依赖类型的对象。
- 基于注解，在 [私有变量](https://zhida.zhihu.com/search?content_id=236390917&content_type=Article&match_order=1&q=%E7%A7%81%E6%9C%89%E5%8F%98%E9%87%8F&zhida_source=entity) 前加类似 "@Inject" 的注解，让工具或者框架能够分析依赖，自动注入依赖。
...
[软件设计](https://zhida.zhihu.com/search?content_id=236390917&content_type=Article&match_order=1&q=%E8%BD%AF%E4%BB%B6%E8%AE%BE%E8%AE%A1&zhida_source=entity) 模式六大原则之一，依赖 [倒置原则](https://zhida.zhihu.com/search?content_id=236390917&content_type=Article&match_order=1&q=%E5%80%92%E7%BD%AE%E5%8E%9F%E5%88%99&zhida_source=entity) ，英文缩写 **DIP** ，全称 Dependence Inversion Principle。
...
为什么依赖抽象而不依赖实现那么重要呢？在复杂的架构里面，合理的抽象能够帮助我们在保持简洁，在领域边界内提 [高内聚](https://zhida.zhihu.com/search?content_id=236390917&content_type=Article&match_order=1&q=%E9%AB%98%E5%86%85%E8%81%9A&zhida_source=entity) ，不同的边界降低耦合，指导项目进行合理的目录结构划分。在实现 SSR 和 CSR 的复合能力的时候，在客户端运行的时候，需要通过 HTTP 去请求数据，而在 [服务端](https://zhida.zhihu.com/search?content_id=236390917&content_type=Article&match_order=1&q=%E6%9C%8D%E5%8A%A1%E7%AB%AF&zhida_source=entity) ，我们只需要直接调用 DB 或者 RPC 就能够获取数据。
...
除了实现了软件工程里面最重要的 [高内聚低耦合](https://zhida.zhihu.com/search?content_id=236390917&content_type=Article&match_order=1&q=%E9%AB%98%E5%86%85%E8%81%9A%E4%BD%8E%E8%80%A6%E5%90%88&zhida_source=entity) ，依赖注入还能够提高代码的可测试性。
...
这样的单元测试虽然能够正常运行，但是由于 Stub 的函数是入侵在 prototype 上面，这是一个全局的副作用影响，会让其他单元测试如果运行到这里受到影响。如果在测试结束的时候，清除了 [sinon](https://zhida.zhihu.com/search?content_id=236390917&content_type=Article&match_order=2&q=sinon&zhida_source=entity) 的副作用，倒是不会影响串行的单元测试，但是就无法进行并行测试了。而使用了依赖注入的方法，就不会有这些问题了。
...
[从 github 的 DI 分类可以查看到一些流行的 DI 工具](https://link.zhihu.com/?target=https%3A//github.com/topics/dependency-injection%3Fl%3Dtypescript) 。

[InversifyJS](https://link.zhihu.com/?target=https%3A//github.com/inversify/InversifyJS): 能力强大的，依赖注入的工具，比较严格的依赖抽象的执行方式；虽然严格的申明很好，但是写起来很重复和啰嗦。

[TSyringe](https://link.zhihu.com/?target=https%3A//github.com/microsoft/tsyringe): 简单易用，继承自 [angular](https://zhida.zhihu.com/search?content_id=236390917&content_type=Article&match_order=1&q=angular&zhida_source=entity) 的抽象定义，比较值得学习。

## 实现基本的能力

为了实现基本的 DI 工具的能力，去接管对象创建，实现 [依赖倒置](https://zhida.zhihu.com/search?content_id=236390917&content_type=Article&match_order=4&q=%E4%BE%9D%E8%B5%96%E5%80%92%E7%BD%AE&zhida_source=entity) 和依赖注入，我们主要实现三个能力：

- [依赖分析](https://zhida.zhihu.com/search?content_id=236390917&content_type=Article&match_order=1&q=%E4%BE%9D%E8%B5%96%E5%88%86%E6%9E%90&zhida_source=entity) ：为了能够创建对象，需要让工具知道有那些依赖。
- 注册创建器：为了支持不同类型的实例创建方式，支持直接依赖、支持抽象依赖、支持工厂创建；不同的上下文注册不同的实现。
- 创建实例：利用创建器将实例创建出来，支持 [单例模式](https://zhida.zhihu.com/search?content_id=236390917&content_type=Article&match_order=1&q=%E5%8D%95%E4%BE%8B%E6%A8%A1%E5%BC%8F&zhida_source=entity) ，多例模式。

假定我们的最终形态是这样的执行代码，如果想要最终的结果的话， [可以点击在线编码的链接](https://link.zhihu.com/?target=https%3A//codesandbox.io/s/di-playground-oz2j9) 。
...
为了能够让 DI 工具能够进行依赖分析，需要开启 TS 的装饰器功能，以及装饰器的 [元数据](https://zhida.zhihu.com/search?content_id=236390917&content_type=Article&match_order=1&q=%E5%85%83%E6%95%B0%E6%8D%AE&zhida_source=entity) 功能。
...
![[../../_resources/教你如何构建自己的依赖注入工具/9aaba3c758790827134c16e2b9f31509_MD5.webp]]
...
仔细去阅读 \_\_decorate 函数的逻辑的话，实际上就是一个 [高阶函数](https://zhida.zhihu.com/search?content_id=236390917&content_type=Article&match_order=1&q=%E9%AB%98%E9%98%B6%E5%87%BD%E6%95%B0&zhida_source=entity) ，为了倒序执行 ClassDecorator 和 Metadata 的 Decorator，翻译一下上面的代码，就等于：
...
然后我们再仔细阅读 \_\_metadata 函数的逻辑，执行的是 Reflect 的函数，就等于代码：
...
![[../../_resources/教你如何构建自己的依赖注入工具/912ceb3ef77fd67243e49eba8e36192d_MD5.webp]]
...
> 反射元数据的提案： [rbuckton.github.io/refl](https://link.zhihu.com/?target=https%3A//rbuckton.github.io/reflect-metadata/%23syntax)
...
![[../../_resources/教你如何构建自己的依赖注入工具/f4119ac9a2036c8f1466f2839b2f10d0_MD5.webp]]
...
![[../../_resources/教你如何构建自己的依赖注入工具/e8beff56ecdb3e9f7681f12bd1430579_MD5.webp]]
...
作为依赖的唯一标记，可以是 String、Symbol、Constructor、或者 TokenFactory。在没有依赖抽象的情况下，其实就是不同的 Constructor 之间直接依赖；String 和 Symbol 是我们在依赖抽象之后会使用到的依赖 ID；而 TokenFactory 是实在想要进行文件 [循环引用](https://zhida.zhihu.com/search?content_id=236390917&content_type=Article&match_order=1&q=%E5%BE%AA%E7%8E%AF%E5%BC%95%E7%94%A8&zhida_source=entity) 的时候，用来进行解析依赖的方案。
...
![[../../_resources/教你如何构建自己的依赖注入工具/d295c5e7fbbc01c6f66967d13001318c_MD5.webp]]
...
这样不是不可以，单个实现单个作用的时候写起来会更简单，但是这里就要说到我们引入 DI 的目的，为了解耦。工厂函数的逻辑判断其实是 [业务逻辑](https://zhida.zhihu.com/search?content_id=236390917&content_type=Article&match_order=1&q=%E4%B8%9A%E5%8A%A1%E9%80%BB%E8%BE%91&zhida_source=entity) 的一部分，本身不属于具体的实现所归属的领域；并且当实现被多个工厂逻辑中使用的时候，这个地方的逻辑就会变得很奇怪。
...
![[../../_resources/教你如何构建自己的依赖注入工具/15181b877607d4c3d0b7583e4825f627_MD5.webp]]

所以这里除了定义一个 [抽象类型](https://zhida.zhihu.com/search?content_id=236390917&content_type=Article&match_order=1&q=%E6%8A%BD%E8%B1%A1%E7%B1%BB%E5%9E%8B&zhida_source=entity) ，同时我们还需要为这个抽象类型定义一个唯一标记，也就是 Token 里面的 string 或者 symbol。我们一般会选择 symbol，这样全局唯一的值。这里可以利用 TS 同名在值和类型的多重定义，当作值和当作类型让 TS 自己去分析。
...
![[../../_resources/教你如何构建自己的依赖注入工具/0c8eaeac777fcb49ca27ea5960657a7e_MD5.webp]]

下面是编译的结果

![[../../_resources/教你如何构建自己的依赖注入工具/f0f01894cf611edbad54f4ee021e46ce_MD5.webp]]
...
![[../../_resources/教你如何构建自己的依赖注入工具/f5d805cc945480a8822b1884901e207b_MD5.webp]]

## 文件循环

除了文件内循环，还有文件之间的循环，比如下面的这个例子。

![[../../_resources/教你如何构建自己的依赖注入工具/5029778012ad150ba0478e34372f09bf_MD5.webp]]
...
因为 JS 的作用域 [内变量](https://zhida.zhihu.com/search?content_id=236390917&content_type=Article&match_order=1&q=%E5%86%85%E5%8F%98%E9%87%8F&zhida_source=entity) 提升，在函数中是能持有变量引用的，只要保证函数在执行的时候，变量已经赋值过了，就能够正确解析依赖了。
...
![[../../_resources/教你如何构建自己的依赖注入工具/050c2debd764a6b43a806d7a7a1250fa_MD5.webp]]
