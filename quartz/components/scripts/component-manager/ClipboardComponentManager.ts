...
> {
   ...
   * 查找页面中的代码块元素
   */
  protected findElements(): NodeListOf<HTMLPreElement> {
   ...
   * 设置页面元素
   */
  protected setupPageElements(): void {
    ...
    const codeBlocks = this.findElements()
  ...
  private setupCodeBlock(preElement: HTMLPreElement, codeElement: HTMLElement): void {
    ...
    this.debugLog("Code block clipboard button set up", preElement)
  }
  ...
  private getSourceCode(codeElement: HTMLElement): string {
    ...
    if (codeElement.dataset.clipboard) {
      try {
        ...
        this.debugLog("Failed to parse clipboard data, using innerText", error)
  ...
  private async handleCopyClick(button: HTMLButtonElement, source: string): Promise<void> {
    ...
    try {
      ...
      this.debugLog("Code copied to clipboard successfully")
    } catch (error) {
      // 复制失败
      this.state.copyStats.failedCopies++

      this.showCopyFeedback(button, false)
      this.debugLog("Failed to copy code to clipboard", error)
  ...
  public resetCopyStats(): void {
    ...
    this.debugLog("Copy stats reset")
  }
  ...
  public async copyText(text: string): Promise<boolean> {
    try {
      ...
      this.state.copyStats.totalCopies++
      this.state.copyStats.failedCopies++
      this.debugLog("Manual copy failed", error)
      return false
    }
  }
