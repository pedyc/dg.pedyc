...
interface MermaidState {
  ...
  mermaidModulePromise: Promise<typeof import("../lazy/mermaid")> | null
}
...
export class MermaidComponentManager extends BaseComponentManager<MermaidConfig, MermaidState> {
  ...
  private async initializeMermaidElements(nodes: HTMLElement[]): Promise<void> {
    ...
    try {
      ...
      // 转换为 NodeListOf<HTMLElement> 格式
      const nodeList = this.createNodeList(nodes)

      // 初始化 Mermaid 图表
      const result = await mermaidModule.initializeMermaid(nodeList)
   ...
   * 预加载 Mermaid 模块
   */
  private preloadMermaidModule(): Promise<typeof import("../lazy/mermaid")> {
    if (!this.state.mermaidModulePromise) {
      this.state.mermaidModulePromise = import("../lazy/mermaid")
    }
   ...
   * 创建 NodeList 对象
   */
  private createNodeList(nodes: HTMLElement[]): NodeListOf<HTMLElement> {
    return {
      ...nodes,
   ...
   * 显示错误状态
   */
  private showErrorState(nodes: HTMLElement[]): void {
    nodes.forEach((node) => {
