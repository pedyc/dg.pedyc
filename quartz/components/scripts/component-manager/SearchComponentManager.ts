...
let searchModulePromise: Promise<typeof import("../search.bundle")> | null = null
...
> {
  ...
  private async initializeSearchElement(element: HTMLElement): Promise<void> {
    ...
    try {
      ...
      searchLoadingStates.set(element, false) // 确保在失败时重置状态
    }
  }
   ...
   * 加载搜索模块
   */
  private async loadSearchModule(): Promise<typeof import("../search.bundle")> {
    if (!searchModulePromise) {
      searchModulePromise = import("../search.bundle")
    }
