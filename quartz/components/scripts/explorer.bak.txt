...
async function setupExplorer(currentSlug: FullSlug) {
  ...
  for (const explorer of allExplorers) {
    ...
    // const storageTree = globalStorageManager.instance.getItem('local', "fileTree")
...
document.addEventListener("prenav", async () => {
  ...
  if (!explorer) return
  const scrollTopKey = CacheKeyFactory.generateUserKey("explorerScrollTop", "scroll_position")
  globalStorageManager.instance.setItem("session", scrollTopKey, explorer.scrollTop.toString())
})
