...
interface Item {
  ...
  content: string
  tags: string[]
  field: string // Added field property based on usage
}
...
async function setupSearch(searchElement: Element, currentSlug: FullSlug, data: ContentIndex) {
  ...
  async function onType(e: HTMLElementEventMap["input"]) {
    ...
    const getByField = (field: string): number[] => {
      const results = searchResults.filter((x: Item) => x.field === field)
...
export async function fillDocument(data: ContentIndex) {
