...
;(() => {
      ...
      ;((V = class a {
        ...
        processURL(e, t = {}) {
          ...
          try {
            ...
            let u = this.generateCacheKey(i.toString(), o)
            return (
              console.debug(`[URLHandler Debug] Cache Key: ${u}`),
              { original: e, processed: i, cacheKey: u, isValid: !0 }
            )
          } catch (c) {
            return {
              original: e,
    ...
    _ = D(() => {
      ...
      ;((F = (a) =>
        a
          ? a
              .toLowerCase()
              .replace(l.CONVENTIONS.FORBIDDEN_CHARS, "")
              .replace(/\s+/g, l.SEPARATOR)
              .replace(/_+/g, l.SEPARATOR)
              .replace(/^_|_$/g, "")
              .substring(0, l.CONVENTIONS.MAX_LENGTH)
          : ""),
        (d = {
          generateContentKey: (a, e) => {
            ...
            if (!t.isValid)
              return (
                ...
                `${l.PREFIXES.CONTENT}invalid_${F(a)}`
              )
            let r = t.cacheKey
            return e ? `${r}${l.SEPARATOR}${e}` : r
          },
          generateSearchKey: (a, e) => {
            let t = F(a)
            return e ? `${l.PREFIXES.SEARCH}${t}${l.SEPARATOR}${F(e)}` : `${l.PREFIXES.SEARCH}${t}`
          },
          generateUserKey: (a, e) => {
            let t = F(a)
            return e ? `${l.PREFIXES.USER}${e}${l.SEPARATOR}${t}` : `${l.PREFIXES.USER}${t}`
          },
          generateFontKey: (a, e) => {
            let t = F(a)
            return e ? `${l.PREFIXES.FONT}${t}${l.SEPARATOR}${e}` : `${l.PREFIXES.FONT}${t}`
          },
          generateSystemKey: (a, e) => {
            let t = F(a)
            return e ? `${l.PREFIXES.SYSTEM}${t}${l.SEPARATOR}${F(e)}` : `${l.PREFIXES.SYSTEM}${t}`
          },
          identifyType: (a) => {
            let e = Object.entries(l.PREFIXES)
            for (let [t, r] of e) if (a.startsWith(r)) return t
            return null
          },
          extractOriginalKey: (a) => {
            let e = Object.values(l.PREFIXES)
          ...
          generateStorageKey: (a, e) =>
            d.identifyType(a)
              ? a
              : `${{ MEMORY: l.PREFIXES.CONTENT, SESSION: l.PREFIXES.CONTENT }[e] || l.PREFIXES.CONTENT}${a}`,
          validateKeyFormat: (a) => {
            let e = [],
              t = []
            return (
              (!a || a.length === 0) &&
                (e.push("\u952E\u4E0D\u80FD\u4E3A\u7A7A"),
                t.push("\u63D0\u4F9B\u4E00\u4E2A\u975E\u7A7A\u7684\u952E")),
              a.length > l.CONVENTIONS.MAX_LENGTH &&
                (e.push(`\u952E\u8FC7\u957F: ${a.length} > ${l.CONVENTIONS.MAX_LENGTH}`),
                t.push("\u7F29\u77ED\u952E\u540D\u6216\u4F7F\u7528\u54C8\u5E0C\u503C")),
              l.CONVENTIONS.FORBIDDEN_CHARS.test(a) &&
                (e.push("\u952E\u5305\u542B\u7981\u7528\u5B57\u7B26"),
                t.push("\u79FB\u9664\u952E\u4E2D\u7684\u7279\u6B8A\u5B57\u7B26")),
              d.identifyType(a) ||
                (e.push("\u952E\u7F3A\u5C11\u5FC5\u9700\u7684\u524D\u7F00"),
          ...
          parseKey: (a) => {
            let e = d.identifyType(a),
              t = Object.values(l.PREFIXES).find((s) => a.startsWith(s)) || null,
              r = d.extractOriginalKey(a),
              n = d.validateKeyFormat(a)
            return { original: r, type: e, prefix: t, isValid: n.isValid }
          },
        }),
        ({
          generateContentKey: je,
          generateSearchKey: Ye,
          generateUserKey: qe,
          generateFontKey: Qe,
          generateSystemKey: Ze,
          identifyType: Je,
          extractOriginalKey: S,
          generateStorageKey: ie,
          validateKeyFormat: et,
          parseKey: tt,
        } = d),
        (rt = {
          identifyType: d.identifyType,
          extractOriginalKey: d.extractOriginalKey,
          generateStorageKey: d.generateStorageKey,
          validateKey: d.validateKeyFormat,
          parseKey: d.parseKey,
        }))
    })
  function v(a) {
    return ue[a] || ue.DEFAULT
  }
  var l,
    ue,
    z,
    ...
    E = D(() => {
      ...
      ;((it = {
        content: d.generateContentKey,
        search: d.generateSearchKey,
        font: d.generateFontKey,
        user: d.generateUserKey,
        system: d.generateSystemKey,
      }),
        (m = {
          ENABLE_MONITORING: !0,
          MONITOR_INTERVAL: 5 * 60 * 1e3,
          ...
          set(e, t, r = 30 * 60 * 1e3) {
            try {
              ...
              let o = { data: t, timestamp: Date.now(), ttl: r, size: n, accessCount: 0 },
                c = this.cache.set(e, o)
              if ((c && (this.currentMemoryUsage -= c.value.size), c))
                this.currentMemoryUsage -= c.value.size
              else {
          ...
          cleanup() {
            try {
              let e = Date.now(),
                ...
                s = 0,
                o = []
              for (let c of this.cache.keys()) {
                let i = this.cache.get(c)
                i && e - i.timestamp > i.ttl && o.push(c)
              }
              for (let c of o) {
                let i = this.cache.delete(c)
              ...
              if (this.currentMemoryUsage > r) {
                let c = this.cache
                  ...
                  .sort((i, u) => u.priority - i.priority)
                for (let { key: i } of c) {
                  if (this.currentMemoryUsage <= r) break
                  let u = this.cache.delete(i)
      ...
      B = class {
        ...
        cleanupNonCriticalResources() {
          ...
          let e = ["click", "popstate"],
            t = [window, document],
            r = []
          ;(this.eventListeners.forEach(({ element: n, type: s, listener: o, options: c }) => {
            if (e.includes(s) && t.some((i) => i === n))
              (r.push({ element: n, type: s, listener: o, options: c }),
                console.log(
                  `[SPA DEBUG] \u4FDD\u7559\u5173\u952E\u4E8B\u4EF6\u76D1\u542C\u5668: ${s} on ${n.constructor.name}`,
                ))
            else
              try {
                ;(n.removeEventListener(s, o, c),
                  console.log(
      ...
      x = class a {
        ...
        static async checkAndCleanupIfNeeded(e) {
          try {
            ...
            m.CONSOLE_WARNINGS && console.warn("\u914D\u989D\u68C0\u67E5\u5931\u8D25:", t)
          }
        }
        ...
        static cleanupStorage(e) {
          try {
            ...
            m.CONSOLE_WARNINGS && console.error("\u6E05\u7406\u5B58\u50A8\u5931\u8D25:", t)
          }
        }
        static findExpiredKeys(e) {
          ...
          for (let n = 0; n < e.length; n++) {
            let s = e.key(n)
            if (!s || !Object.values(l.PREFIXES).some((i) => s.startsWith(i))) continue
            let c = e.getItem(s)
            c && this.isExpiredItem(c, r) && t.push(s)
          }
          return t
        }
        ...
        static emergencyCleanup(e) {
          try {
            ...
            m.CONSOLE_WARNINGS && console.error("\u7D27\u6025\u6E05\u7406\u5931\u8D25:", t)
          }
        }
    ...
    Z = D(() => {
      ...
      N = class a {
            ...
            let e = { memory: 0, session: 0, local: 0 },
              t = (r, n, s) => {
                ...
                for (let o = 0; o < r.length; o++) {
                  let c = r.key(o)
                  if (!c || !d.identifyType(c)) continue
                  let u = r.getItem(c)
                  if (!u) continue
                  let h = S(c)
                  ...
                  this.referenceMap.set(h, g)
                  let f = this.calculateHash(u)
                  ;(this.contentHashMap.has(f) || this.contentHashMap.set(f, h), e[n]++)
                }
              }
        ...
        set(e, t, r = void 0) {
          ...
          let c = this.selectOptimalLayer(t, r),
            i = ie(e, c)
          if (this.storeContent(i, t, c)) {
            let h = {
              storageLayer: c,
              storageKey: i,
              refCount: 1,
  ...
  var $,
    ...
    ne = D(() => {
      ...
      ;(($ = class {
        ...
        generateCacheKey(...e) {
          return d.generateSystemKey(this.config.cacheConfig.prefix, ...e)
        }
        generateUserCacheKey(e) {
          return d.generateUserKey(this.config.cacheConfig.prefix, e)
        }
        generateContentCacheKey(e) {
          return d.generateContentKey(e)
        }
        setStorageItem(e, t) {
  ...
  var Ce = Te((nr, De) => {
    ...
    function Ke(a) {
      if (((a = a || {}), a.circles)) return He(a)
      let e = new Map()
      if (
        (e.set(Date, (o) => new Date(o)),
        e.set(Map, (o, c) => new Map(r(Array.from(o), c))),
        e.set(Set, (o, c) => new Set(r(Array.from(o), c))),
    ...
    function He(a) {
      ...
      if (
        ...
        r.set(Map, (i, u) => new Map(s(Array.from(i), u))),
        r.set(Set, (i, u) => new Set(s(Array.from(i), u))),
        a.constructorHandlers)
      )
      ...
      function c(i) {
        if (typeof i != "object" || i === null) return i
        if (Array.isArray(i)) return s(i, c)
        if (i.constructor !== Object && (n = r.get(i.constructor))) return n(i, c)
        let u = {}
        ;(e.push(i), t.push(u))
        for (let h in i) {
          let g = i[h]
  ...
  var k,
    ye = D(() => {
      ...
      k = class extends $ {
        ...
        async onInitialize() {
          ;((this.state.currentSlug = Fe(window)),
        ...
        onSetupPage(e) {
          ...
          ;(this.cleanupObserver(),
            this.config.enableLazyLoad
              ? this.setupLazyLoading(e)
              : this.initializeMermaidElements(e),
        ...
        async initializeMermaidElements(e) {
          ...
          if (!this.state.loadingStates.get(t)) {
            ...
            try {
              ;(await (await this.preloadMermaidModule()).initializeMermaid(),
                this.log(`Mermaid initialized successfully for ${e.length} elements`))
            } catch (r) {
              this.error("Failed to load mermaid module:", r)
            } finally {
        ...
        preloadMermaidModule() {
          return (
            this.state.mermaidModulePromise ||
              (this.state.mermaidModulePromise = Promise.resolve().then(() => (Me(), Ae))),
            this.state.mermaidModulePromise
          )
        }
        schedulePreload() {
